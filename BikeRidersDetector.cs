        ...
        private void loadVideo()
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    curFrameCount = 0;
                    bikeCounter = 0;
                    preMillis = getCurrentMillis();
                    UpdateText("Bike Counter = 0");
            ...
            if (frame != null)
            {
                ...
                if (contours.Size > 0)
                {
                    ...
                    for (int i = 0; i < contours.Size; i++)
                    {
                        ...
                        if (area > 300 && rect.Y>200 && area<3500 && rect.X>100 && rect.X<400 && rect.Width<(rect.Height-10))
                        {
                            if (center_y >= 410 && center_y<420)
                            {
                                CvInvoke.Rectangle(frame, rect, new MCvScalar(255, 0, 0), 6);
                                
                                if ((getCurrentMillis() - preMillis) > 100)
                                {
                                    
                                    bikeCounter++;
                                    detectFlag++;
                                    UpdateText("Bike Count = " + bikeCounter);                                                                
                                }
                            }
                            else
                            {
                ...
                CvInvoke.Line(frame, new Point(0,400), new Point(640,400), new Bgr(Color.Red).MCvScalar, 12);
